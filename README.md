...
| --- | --- | --- |
| App | Yes | The Dify chat app whose logs you want to evaluate. On each run the plugin checks that the App API Key below actually belongs to this app (and that it is a chat app), and stops with an error if not. |
...
| Save Report to Knowledge Base | No | When on, each run's report is saved as a new Document in a Dify Knowledge Base, giving you a persistent, searchable history. The document holds the summary plus every evaluated message (verdicts, issue, corrected answer). Runs that evaluate nothing aren't saved. Default: off. |
